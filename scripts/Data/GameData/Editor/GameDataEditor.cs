...
public class GameDataEditor : UnityEditor.AssetModificationProcessor {
	...
	static void SaveGlobalData(){
		...
		GameData.Instance.QuestData.Quests.UpdateItem (new QuestInfoGameData (-100, -100));
		GameData.Instance.QuestData.Quests.UpdateItem (new FetchQuestInfoGameData (-101, -100));
	...
	static AreaGameData UpdateAreaData(LevelSettings levelSettings){
        ...
        var areaData = GameData.Instance.NavigationData.Areas.GetItem(levelSettings.areaID);
        if (areaData == null) {
        ...
        GameData.Instance.NavigationData.Areas.UpdateItem (areaData);
		return areaData;
	}
	...
	static void UpdateClientData(AreaGameData currentArea){
		...
		foreach (var client in clients) {
			...
			var currentData = GameData.Instance.QuestData.Clients.GetItem(client.clientID);
			if(currentData != null){
			...
			GameData.Instance.QuestData.Clients.UpdateItem(clientData);
		}
		...
		foreach (var origClient in originalClients) {
			if(origClient.AreaID == currentArea.AreaID){
				if(!foundClients.Contains(origClient.ClientID)){
					...
					GameData.Instance.QuestData.Clients.RemoveItem(origClient.ClientID);
				}
			}
		}
	}
	...
	static void UpdateQuestData(){
		...
		foreach (var quest in quests) {
			...
			GameData.Instance.QuestData.Quests.UpdateItem(questData);
		}
	}
	...
	static void UpdateChallengeData(){
		...
		foreach (var client in clients) {
			if(client.clientData){
				...
				GameData.Instance.ProgressionData.Challenges.UpdateItem(challenge);
			}
		}
	}
