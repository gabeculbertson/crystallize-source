...
namespace Crystallize.Data {
	public class WordStore {
		...
		public bool AddFoundWord(PhraseSegmentData phrase){
			if (!FoundWordIDs.Contains (phrase.ID)) {
				FoundWordIDs.Add(phrase.ID);
				RemoveObjectiveWord(phrase);
				return true;
			}
			return false;
		}
		...
		public bool ContainsUnlockedWord(PhraseSegmentData phrase){
			return UnlockedWordIDs.Contains(phrase.ID);
		}

		public bool AddObjectiveWord(PhraseSegmentData phrase){
			if (!ObjectiveWordIDs.Contains (phrase.ID) && !FoundWordIDs.Contains(phrase.ID)) {
				ObjectiveWordIDs.Add(phrase.ID);
				return true;
			}
			return false;
		}
