...
public class ConversationClientPanelUI : MonoBehaviour {
    ...
    void HandleOnDialogSuccess(object sender, System.EventArgs e) {
        PlayerData.Instance.FriendData.SetFriendState(conversationClients[0].clientData.ID, 1);
        UnlockWords();
    }
    ...
    public void AddClient(ConversationClientData clientData) {
        PlayerData.Instance.FriendData.SetFriendState(clientData.ID,
                                                                 PlayerData.Instance.FriendData.GetFriendData(clientData.ID).FriendLevel);
        ...
        if (objectivePanel) {
            ...
            if (conversationClients.Count > 0) {
                objectivePanel.Set(conversationClients[0].PhraseData,
                                   PlayerData.Instance.FriendData.GetFriendData(conversationClients[0].clientData.ID).FriendLevel == 1);
    ...
    void Load() {
        foreach (var c in PlayerData.Instance.FriendData.Friends) {
