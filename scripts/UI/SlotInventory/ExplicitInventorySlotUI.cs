...
public class ExplicitInventorySlotUI : UIMonoBehaviour, IPhraseDropHandler, IPhraseDropEvent, IPointerEnterHandler,
    ...
    void RefreshSlotState() {
        ...
        if (rankUpReady && PlayerManager.Instance.State == PlayerState.Exploring) {
            phraseText.color = Color.Lerp(Color.black, Color.gray, Mathf.PingPong(Time.time * 2f, 1f));
        } else {
            phraseText.color = Color.black;
        }

        if (PlayerData.Instance.WordStorage.ContainsFoundWord(Word.WordID)) {
            slot.GetComponent<Image>().color = GUIPallet.Instance.GetColorForWordCategory(word.GetPhraseCategory());
            slot.GetComponentInChildren<Text>().font = GUIPallet.Instance.defaultFont;
            if (hovering) {
                targetAlpha = 0.2f;
            } else {
                targetAlpha = 1.5f;
    ...
    public void AcceptDrop(IWordContainer phraseObject) {
        if (Word != null) {
            if (phraseObject.Word.WordID == Word.WordID) {
                if (!PlayerData.Instance.WordStorage.FoundWords.Contains(Word.WordID)) {
                    Fulfill();
    ...
    void TrySRS() {
        ...
        if (ObjectiveManager.main.IsWordFound(word.WordID)) {
            if (PlayerManager.Instance.State == PlayerState.Exploring) {
                Action a = null;
