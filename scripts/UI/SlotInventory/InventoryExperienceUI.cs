...
public class InventoryExperienceUI : MonoBehaviour, IExperienceUI {
	...
	void Update () {
		var inventoryState = PlayerManager.main.playerData.InventoryState;
		...
		levelText.text = PlayerManager.main.playerData.InventoryState.Level.ToString();
		experienceText.text = string.Format ("{0}/{1}", 
		                                    (int)(currentEnergy * PlayerManager.main.playerData.InventoryState.NextLevelExperience),
		                                    PlayerManager.main.playerData.InventoryState.NextLevelExperience);
